...
# Opening statements
my $usage = "Usage: perl $0 -o <ortholog file (E.g. PEP.RBH.OrthoClusters, all_orthomcl.out, Orthogroups.csv)>\n
...
foreach($opt_o, $opt_r, $opt_p) { die "ERROR: $_ not recognised as a file : $!\n" if(! -f $_); }
die "-t not equal to OMCL or RBH: $opt_t\n" unless(($opt_t eq 'OMCL') || ($opt_t eq 'RBH') || ($opt_t eq 'Orthofinder'));
...
# Make genome codes for Orthofinder (if Orthofinder)
elsif($opt_t eq 'Orthofinder') {
	$genome_codes = "for_orthofinder.genome_codes";
	open my $ofh, '>', $genome_codes or die "Cannot open $genome_codes : $!\n";
	my @genomes = $data_manager->get_genome_list();
	for(my $i=0; $i<scalar(@genomes); $i++) {
		my $genome = $genomes[$i];
		my $annotation = $data_manager->get_data_dump_filename($genome, 'Annotation');
		$annotation =~ s/\.annotation\.gff[3]$//i;
		my $base_filename = basename($annotation);
		print $ofh "$genome\t$base_filename\n";
	}
	close $ofh;
}
# Not needed for RBH
...
foreach my $curr_transcript ( keys %{$all_proteins}) {
	if(! $orts->has_at_least_one_ort( $curr_transcript ) ) {
...
# Combine 'cos it's useful (but don't append to an already made file)
if(-e "$orthocluster_out_dir/$ortholog_analysis_name.clusters_and_uniques") { `rm $orthocluster_out_dir/$ortholog_analysis_name.clusters_and_uniques`; }
...
sub count_clustered_genes_per_genome {
	...
	warn "count_clustered_genes_per_genome...\n";
...
sub parse_annot {
	...
	open my $fh, '<', $annot_file or die "Cannot open $annot_file : $!\n";
	while (my $line=<$fh>) {
		next if($line =~ m/^#/);
